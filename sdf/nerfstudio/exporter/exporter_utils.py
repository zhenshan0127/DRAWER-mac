...
def remesh(mesh):
    ...
    edge_len = edge_len[np.isfinite(edge_len)]
    mean_edge_len = np.mean(edge_len / 3)

    print("mean_edge_len: ", mean_edge_len)
...
def get_mesh_from_filename(filename: str, target_num_faces: Optional[int] = None) -> Mesh:
    ...
    print("remesh..")
    verts, faces, normals = remesh(mesh)

    # return get_mesh_from_pymeshlab_mesh(mesh_remeshed)
    vertices = np.array(verts).reshape(-1, 3).copy()
    faces = np.array(faces).reshape(-1, 3).copy()
    normals = np.array(normals).reshape(-1, 3).copy()
